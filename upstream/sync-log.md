...
- `freee-monthly-check`：増減チェックのバグ修正を上流から先行取り込み（freee `/reports/trial_pl` の `closing_balance` は期首からの累計→当月発生額 `closing - opening` ベースに修正、テスト2件追加、SKILL.mdに制約を記載）。`last_synced_commit`は据え置き（次回の通常syncで差分は適用済みとして流れる）
