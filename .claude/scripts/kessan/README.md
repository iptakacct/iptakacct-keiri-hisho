...
- `import-bank` は、追加が0件（全部が取り込み済み）でも処理したファイルを `import-log.csv` に1行記録する。`対象期間` は事業年度内・金額0以外の全部の行の期間、`件数`・`入金合計`・`出金合計` は新しく追加した行の分。同じ記録名のファイルを再実行しても、記録は1行のまま増やさない（`unimport` の期間の重なりの確認で、取り込んだ資料が漏れないようにするため）。
