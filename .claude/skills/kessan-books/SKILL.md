...
**禁止事項**
- `staging.csv`・`journal.csv`・`evidence.csv`・`import-log.csv` を直接編集しない（読むのはよい）。AIが書くのは `extracted/*.json`（読み取り結果）と `output/set-accounts-YYYYMMDD.json`（科目候補）だけで、書き換えは必ずコマンドを通す
- `set-accounts` で `判定=確立済み` を指定しない（スクリプトが拒否する）。AIの判断では自動承認しない
- 「証憑から計上済みの仕訳と金額・日付が近い（二重計上の疑い）」「未払候補の証憑と金額が一致（支払の可能性）」「証憑の重複の疑い」が付いた行は、AIの判断だけで承認・削除しない。必ずオーナーに同一取引かどうかを確認する
...
- 「証憑から計上済みの仕訳と金額・日付が近い（二重計上の疑い）」「未払候補の証憑と金額が一致（支払の可能性）」が付いた明細行（銀行CSV・通帳・出納帳の取り込み時に付く）は、AIの判断だけで承認しない。オーナーに同一取引かどうかを確認してから④・⑥に進む
- 「残高が連続しないページ」：資料の該当ページを読み直す。該当ページの行は `読み取り信頼度=低` で取り込まれている（自動では直らない）ので、**その行を承認しない**（④・⑥で `set-accounts`・`apply-review`・`approve` の対象にせず、要確認のまま残す）。読み違いが疑われる場合も自分でCSVを書き換えたり消したりしない
  - オーナーに、該当ページと読み違いの疑いを報告する。1件の資料の取り込みを取り消す（やり直す）コマンドはまだ無い（段階3以降の課題）。読み直しが必要なら、その後どう進めるか（そのまま要確認で残す・別途調整仕訳を検討する等）はオーナーが判断する
...
- 要確認の要点（番号・日付・金額・摘要・科目候補・理由）。件数が多ければ「読み取り信頼度が低い」「明細に該当なし」「新規の取引先」「二重計上の疑い」などの理由ごとにまとめる
...
- **Excelで確認してもらった場合**：`承認` 列に `済`、直してほしい行は `修正メモ` を書いて保存・閉じてもらい、
  ...
  ```
  修正メモが出たら、内容に沿って④（`set-accounts`）で直し、⑤から繰り返す。修正メモが付いた行は、たとえ`承認=済`でも承認しない（自動承認済みの行に修正メモが付いた場合は承認が外れて要確認に戻る）。**確認用Excelを出した後に `set-accounts` などで内容（日付・借方・貸方・金額・摘要）が変わった行は、`承認=済` のままでも承認されない**（非表示の「確認用指紋」列で内容の変化を検知する）。そうした行は `review` を出し直して改めて確認してもらう
