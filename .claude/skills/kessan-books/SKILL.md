...
**役割分担（必ず守る）**
- **AI（このスキル）**：資料を読んで `extracted/<資料のファイル名>.json` を書く／要確認の行に科目候補と理由を付ける（`output/set-accounts-YYYYMMDD.json`）／結果をオーナーに報告する
...
- **内容は同じでも「別の」資料ファイル**（同じ領収書を撮り直した・再スキャンした等）を取り込むと、取り込み自体は止めず、要確認理由に「証憑の重複の疑い」を付けて記録する。自動では削除・除外しないので、同一の証憑かどうかをオーナーに確認する
