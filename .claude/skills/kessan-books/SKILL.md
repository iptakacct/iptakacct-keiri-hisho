...
**取り込みの順番**：銀行・カードのCSV（`import-bank`）と通帳を先に取り込み、領収書・請求書はその後にする（証憑は取り込み済みの明細と突き合わせるため。明細より先に取り込むと「明細に該当なし」になる）。`import-extracted` は1回の実行の中では、渡した読み取り結果ファイルの順番によらず通帳・出納帳を先に、領収書・請求書を後に自動で処理するので、`--file` に複数まとめて渡してもよい。ただし銀行・カードのCSVの取り込み（`import-bank`）は別コマンドなので、それは事前に済ませておく。
...
- 空欄でよい項目は `日付原文`・`補助`・`補助候補`・`メモ`・（通帳の）`繰越残高` だけ。通帳・出納帳の行の `摘要` はキー自体は必要だが値は空文字でもよい。それ以外の項目（領収書・請求書の `取引先`・`内容`・`科目候補`・`支払方法の推定`・`自信度` を含む）は必ず埋める。1件でも欠けている・空のファイルはまるごと取り込めない（`import-extracted` が形式エラーで返す）
