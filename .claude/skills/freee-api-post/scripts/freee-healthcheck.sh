#!/usr/bin/env bash
# freeeのrefresh_token（90日で失効）を切らさないための定期ヘルスチェック。
# companies.json記載の全社についてトークン更新＋軽い読み取りを行うだけで、
# 業務データの書き込みは一切行わない。月1回、Windowsタスクスケジューラーから起動する想定。
#
...
#
# 2026-08-22更新：当初はclaude -p + Slack MCPで投稿していたが、その方式だとオーナー自身の
# アカウントとして投稿されるため、Slackの仕様で自己送信メッセージのメンション通知が抑制され、
# 失敗に気づけない。daily-digest.ps1・payroll-reminder.shと同じくWebhook投稿に統一した。
set -euo pipefail
...
# ラッパー自身の起動・成否だけを記録する。会社ごとの成否は freee_cli.py が
# 各インスタンスの work/freee-api-post.log に書くので、ここでは重複させない。
log() { echo "[$(timestamp)] $1" >> "$LOG_FILE"; }

# JSON文字列リテラルとして安全な形にエスケープする。
# 通知本文にはfreeeのエラーレスポンス（`HTTP 401: {"error":"invalid_grant"}` のような
# ダブルクォート・改行を含む文字列）がそのまま載るため、これを通さないと
# ペイロードのJSONが壊れてWebhookが400を返し、肝心の失敗通知自体が届かなくなる。
json_escape() {
  local s="$1"
  s="${s//\\/\\\\}"
  s="${s//\"/\\\"}"
...
# 全体通知チャンネル にBot（Incoming Webhook）経由で通知する。
# 引数: 投稿するメッセージ本文（メンションを含めるかは呼び出し側の責任）。
# 成功なら0、失敗なら非0を返す（呼び出し側の終了コードは上書きしない）。
notify_general() {
  local message="$1"
  ...
  # 注意（2026-08-22判明）：日本語を含むJSONをcurlの--dataにコマンドライン引数として直接渡すと、
  # Windows環境ではコンソールのコードページ変換で文字化けすることがある。
  # 一時ファイルに書き出し、curl --data-binary @file の形で渡すことで回避する。
  payload_file="$(mktemp)"
...
if ! PYTHON_BIN="$(find_python_bin)"; then
  ...
  notify_general "freeeヘルスチェックが失敗しました（pythonが見つかりません）。手動での確認が必要です。" || true
  exit 1
fi

OUTPUT_FILE="$(mktemp)"

if "$PYTHON_BIN" "$SKILL_DIR/lib/freee_cli.py" \
...
notify_general "【freee APIヘルスチェック失敗】${FAILURE_DETAIL} — 該当会社のトークンが失効している可能性があります。.claude/skills/freee-api-post/setup-guide.md の手順に沿って freee-authorize-start.sh から再認可してください。" || true
